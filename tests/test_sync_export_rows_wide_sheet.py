...
class BuildExportRowsGridTests(unittest.TestCase):
    ...
    def test_transform_operator_record_formats_dates_and_keeps_aggregated_metrics_numeric(self):
        ...
        self.assertEqual(row["bounce_rate"], Decimal("3"))
        self.assertEqual(row["page_depth"], Decimal("25"))
        self.assertEqual(row["time_on_site_seconds"], Decimal("150"))
        self.assertEqual(row["robot_rate"], Decimal("2"))
    ...
    def test_build_export_rows_grid_uses_preaggregated_operator_rows(self):
        columns = [
            "topic",
            "report_date",
            "report_date_from",
