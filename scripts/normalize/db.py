...
def delete_existing_rows_for_run(conn, run_date: str):
    return turso_writes.delete_existing_rows_for_run(conn, run_date)
...
def insert_fact_rows(conn, rows):
    return turso_writes.insert_fact_rows(conn, rows)


def insert_fact_dimensions(conn, rows):
    return turso_writes.insert_fact_dimensions(conn, rows)


def insert_fact_metrics(conn, rows):
    return turso_writes.insert_fact_metrics(conn, rows)
